...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    // Load profile data with CORRECT key
    ...
    // Load user's projects with CORRECT key
    ...
    // Load user's posts with CORRECT key
  ...
  const downloadResume = () => {
    ...
    if (!user?.isPremium) {
      toast.error('Resume download requires Premium subscription');
      return;
    }
    ...
    const resumeContent = `
      ...
      EXTERNAL PROJECTS:
      ${profileData.otherProjects.map(p => `- ${p.title}: ${p.description} (${p.url})`).join('\n')}
      
      Generated by ProofBoard - ${new Date().toLocaleDateString()}
    `;
    ...
    link.href = url;
    link.download = `${profileData.fullName || 'resume'}-resume.txt`;
    link.click();
